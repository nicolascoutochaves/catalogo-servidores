...
int select_program(int opt, char* input_file, char* profile_path, char* output_file, char* encoding) {
    ...
    printf("7. Clear indices\n");
    printf("8. Clear preprocessed files\n");
    printf("9. Clear binary files\n");
    printf("10. Clear all files\n");
    printf("11. Read new data\n");
    printf("12. Exit\n");
    ...
    case OPTION_SORT_BY_FIELD:
        printf("Insert the field to sort by (ID, Name, Gross_Salary, Discounts, Net_Salary):\n");
        ...
        int descending;
        if(scanf("%d", &descending) != 1 || (descending != 0 && descending != 1)) {
        ...
        clock_gettime(CLOCK_MONOTONIC, &t1);
        puts("\n\nSorting...\n");
        sort_by_field(field, descending, output_file);
        break;
    ...
    case OPTION_READ_NEW_DATA:
        ...
        if (parsed < 4 || strcmp(tmp_enconding, "0") == 0) {
            // Encoding = Encoding do perfil (nao muda nada)
        }
        else if (!is_valid_encoding(tmp_enconding)) {
            fprintf(stderr, "Invalid encoding. Use 'utf8' or 'latin1'.\n");
            return OPTION_LOOP;
        }
        else {
